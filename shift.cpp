...
#pragma HLS DATAFLOW
		...
		conv2d_3x3<D + 2, C, CHAN_0, 1, REX, REP>(s_pad, kernel, s_conv);
		MulChan::_bias_add<D,CHAN_0, REX, REP>(s_conv, bias0, s_convBias);
		MulChan::_relu<D,CHAN_0, REX, REP>(s_convBias, s_relu);
		...
		MulChan::_avg_pool< DIM_2, CHAN_2, 8, 16 * REX, REP>(s_act8, s_pool);
		// fully-connected 
		hls::stream<DataType> s_fc[N], s_fcBias[N], s_act[N];
		MulChan::_matMul<DIM_3, CHAN_2, N, 16 * REX, REP>(s_pool, s_fc, p_9);
		MulChan::_bias_add<1, N, 16*REX, REP>(s_fc, bias_9, s_act);
