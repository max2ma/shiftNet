...
def shift_kernel(size):
	dx = np.zeros(size, dtype=np.int)
	dy = np.zeros(size, dtype=np.int)
	k = np.zeros([3,3, size, size], dtype=np.int)
	d = np.zeros(size, dtype=np.int)
	G = (size // 9) * 9
	for i in range(size):
		if i < G:
			d[i] = i%9 - 4
...
def main():
	...
	np.savetxt(r"params/inputs_batch_0", tensor.reshape([1, -1]), delimiter=',')
	
	kernel = np.random.normal(size=(3, 3, C, 16))
	np.savetxt(r"params/t_k", kernel.reshape([1, -1]), delimiter=',')
	bias0 = np.random.normal(scale=1/np.sqrt(16), size=16)
	np.savetxt(r"params/bias0", bias0.reshape([1, -1]), delimiter=',')

	
	def block(idx ,fmap, in_c, out_c, ex,  stride):
		mid_c = out_c * ex;
		p0 = np.random.normal(scale=1/np.sqrt(in_c),size=(1, 1, in_c, mid_c))
		np.savetxt(r"params/p0_%d"%idx, p0.reshape([1, -1]), delimiter=',')
		t_conv0 = tf.nn.conv2d(fmap,p0,strides=[1,1,1,1],
					padding="VALID")
		bias0 = np.random.normal(scale=1/np.sqrt(mid_c), size=mid_c)
		np.savetxt(r"params/bias0_%d"%idx, bias0.reshape([1, -1]), delimiter=',')
		t_relu=tf.nn.relu(tf.nn.bias_add(t_conv0, bias0))
		t_shift=shift(idx, t_relu, mid_c)
		p1 = np.random.normal(scale=1/np.sqrt(out_c),size=(1, 1, mid_c, out_c))
		...
		if in_c!=out_c or stride !=1 :
			...
			t_shortcut = tf.nn.conv2d(fmap, p2, strides=[1, stride, stride, 1],
					padding="VALID")
			bias2 = np.random.normal(scale=1/np.sqrt(out_c), size=out_c)
			np.savetxt(r"params/bias2_%d"%idx, bias2.reshape([1, -1]), delimiter=',')
			t_act = t_act+tf.nn.bias_add(t_shortcut,bias2)
		else:
			t_act = t_act + fmap
		return t_act
	...
	with g.as_default():
		...
		t_conv = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(t_im, kernel,
						strides=[1,1,1,1], padding="SAME"),bias0))
		...
		t_pool = tf.nn.avg_pool(t_act8, [1, 8, 8, 1], [1, 8, 8, 1], padding= "VALID")
		t_arr = tf.reshape(t_pool, [1, -1])
		...
		t_cifar = tf.nn.bias_add(t_fc, bias_9)
	with tf.Session(graph=g) as sess:
		n_cifar= sess.run(t_act0)
		np.savetxt(r"params/outputs_batch_0", n_cifar.reshape([1, -1]), delimiter=',')
