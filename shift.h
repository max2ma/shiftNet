...
#pragma HLS PIPELINE II=IIs
	...
	template< int D, int C, int N, int S, int IIs, int REP, typename T>
...
#pragma HLS PIPELINE II=IIs
	...
	template< int D, int C, int N, int IIs, int REP, typename T>
...
#pragma HLS PIPELINE
	...
	template<int D, int IP, int E, int IIs, int REP, typename T>
...
#pragma HLS INLINE
			hls::stream<T> f_in0[IP], f_in1[IP];
#pragma HLS STREAM VARIABLE=f_in1 DEPTH=D*2*REP
			_duplicate<D, IP, REP>(input, f_in0, f_in1);

			hls::stream<T> f_conv0[MP], f_bias0[MP], f_shift[MP],f_conv1[IP], f_bias1[IP], f_relu[MP], f_relu1[IP];
	...
	template<int D, int S_conv, int IP, int E, int OP, int IIs, int REP, typename T>
...
#pragma HLS STREAM VARIABLE=f_in1 DEPTH=D*2*REP
			...
			hls::stream<T> f_conv0[MP],f_bias0[MP], f_shift[MP], f_relu[MP];
			hls::stream<T> f_conv1[OP],f_bias1[OP], f_relu1[OP], f_shortcut[OP], f_bias2[OP];
